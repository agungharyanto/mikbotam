...
if (!isset($_SESSION["Mikbotamuser"])) {
	...
	if (isset($_POST['saveprofile']) && !empty($_POST['name'])) {
		...
		if ($lock_macs == 'enable') {
			$on_login = ':put (",' . $lock_macs . ',' . $validdity . ',");{:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validdity . ');:local macadd $"mac-address";:local ipaddresslocal $"address";[/ip hotspot user set mac-address=$"macadd" comment="used" [find where name=$user]];[/system scheduler add disabled=no interval=$uptime name=$user on-event= "[/ip hotspot active remove [find where user=$user]];[/ip hotspot user remove [find where name=$user]];[/ip hotspot cookie remove [find user=$user]];[/sys sch re [find where name=$user]]" start-date=$date start-time=$time];}}';
		} else {
			$on_login = ':put (",' . $lock_macs . ',' . $validdity . ',");{:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validdity . ');:local macadd $"mac-address";:local ipaddresslocal $"address";[/ip hotspot user set comment="used" [find where name=$user]];[/system scheduler add disabled=no interval=$uptime name=$user on-event= "[/ip hotspot active remove [find where user=$user]];[/ip hotspot user remove [find where name=$user]];[/ip hotspot cookie remove [find user=$user]];[/sys sch re [find where name=$user]]" start-date=$date start-time=$time];}}';
		}
			...
			<div class="card-body pd-sm-15">
				<form method="post" action="">
					...
					<div class="row mg-t-8">
						...
						<div class="col-sm-8 mg-t-10 mg-sm-t-0">
							<input type="text" class="form-control" name="valid_time" value="" data-toggle="tooltip" data-placement="top" title="Masa Berlaku Contoh : 30d = 30hari, 12h = 12jam, 30m = 30menit" placeholder="1d" >
						</div>
					</div>
					...
					<div class="row mg-t-10 mg-b-10">
						...
						<div class="col-sm-8 mg-t-10 mg-sm-t-0">
							<select class="form-control select2id" name="unrok" data-placeholder="Select Type" data-toggle="tooltip" data-placement="top" title="Lock username dengan Mac">
