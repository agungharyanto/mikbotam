...
$mkbot->cmd('/start|/Start', function () {
    ...
    $info         = bot::message(Selamat Datang di Voucher PLB);
    $ids          = $info['chat']['id'];
 ...
 if (!empty($jumlah)) {
            if (has($idtelegram) == false) {
                ...
                if (preg_match('/^[0-9]+$/', $jumlah)) {
                    if (strlen($jumlah) < 7) {
                    	//jika user belum terdaftar
                    	     $text .= "Terima kasih $nametelegram \n Permintaan Deposit anda sudah kami terima, Mohon ditunggu Permintaan akan segera kami prosses  \n";
                          $textsend        = ""; 
...
$mkbot->on('callback', function ($command) {
    ...
    if (has($id)) {
        ...
        $text .= "@$usernamepelanggan permintaan deposit  sebesar ".rupiah($cek). " sudah kami terima, Mohon ditunggu Permintaan anda akan segera kami prosses \n";
            $options = [
                'chat_id' => $chatidtele,
                'message_id' => (int) $message['message']['message_id'],
                'text' => $text,
                'reply_markup' => json_encode([
                    'inline_keyboard' => [
                        [
                            ['text' => 'Terima kasih', 'callback_data' => 'used'],
                        ], ]]),
                'parse_mode' => 'html'
